import dagre from '../dagre.bundle.js'
...
function estimateNodeSize(id: string, label: string, shape: string): { width: number; height: number } {
  // Strip HTML tags for text measurement, split by <br> for multi-line
  const plainLines = label
    .split(/<br\s*\/?>/gi)
    .map(line => line.replace(/<[^>]+>/g, '').trim())
  const numLines = Math.max(plainLines.length, 1)
  const widestLine = plainLines.reduce((max, line) => {
    const w = estimateTextWidth(line, FONT_SIZES.nodeLabel, FONT_WEIGHTS.nodeLabel)
    return Math.max(max, w)
  }, 0)

  const lineHeight = FONT_SIZES.nodeLabel * 1.4
  let width = widestLine + NODE_PADDING.horizontal * 2
  let height = lineHeight * numLines + NODE_PADDING.vertical * 2
...
function extractPositionedGraph(
  ...
  // Spread out overlapping edge endpoints at the same node side
  spreadOverlappingEndpoints(edges, nodes)

  // Push edge labels out of subgraph header bands so they don't overlap
  nudgeEdgeLabelsFromHeaders(edges, flatGroups, headerHeight)
...
/**
 * Spread out edge endpoints that overlap at the same side of the same node.
 *
 * When multiple edges connect to the same side of a node (e.g. multiple arrows
 * arriving at the top of a node), clipEndpointsToNodes routes them all to the
 * node's center on that side, causing visual overlap. This function detects
 * these collisions and distributes endpoints evenly along the node edge.
 */
/**
 * Push edge labels that overlap with subgraph header bands downward.
 *
 * Dagre places edge labels at the midpoint of the edge, which may land inside
 * a subgraph's header band. The header band is added in post-processing (not
 * visible to dagre), so dagre doesn't account for it.
 */
function nudgeEdgeLabelsFromHeaders(
  edges: PositionedEdge[],
  groups: PositionedGroup[],
  headerHeight: number,
): void {
  const labelHalfH = (FONT_SIZES.edgeLabel + 16) / 2

  for (const edge of edges) {
    if (!edge.labelPosition || !edge.label) continue

    const labelTop = edge.labelPosition.y - labelHalfH

    for (const group of groups) {
      const headerBottom = group.y + headerHeight

      // Check if label overlaps with the header band
      if (
        labelTop < headerBottom &&
        edge.labelPosition.y > group.y &&
        edge.labelPosition.x >= group.x &&
        edge.labelPosition.x <= group.x + group.width
      ) {
        // Push the label above the header band (outside the subgroup)
        edge.labelPosition.y = group.y - labelHalfH - 4
      }
    }
  }
}

function spreadOverlappingEndpoints(
  edges: PositionedEdge[],
  nodes: PositionedNode[],
): void {
  // Build a node lookup by ID (top-left coords)
  const nodeMap = new Map<string, PositionedNode>()
  for (const n of nodes) nodeMap.set(n.id, n)

  // Group endpoints by (nodeId, side) — track both source and target ends
  type EndpointRef = { edgeIdx: number; end: 'source' | 'target' }
  ...
  for (let i = 0; i < edges.length; i++) {
    ...
    if (tNode) {
      ...
      const side = detectSide(last, tNode)
      if (side) {
        const key = `${edge.target}:${side}`
        if (!groups.has(key)) groups.set(key, [])
        groups.get(key)!.push({ edgeIdx: i, end: 'target' })
      }
    }
    ...
    if (sNode) {
      ...
      const side = detectSide(first, sNode)
      if (side) {
        const key = `${edge.source}:${side}`
        if (!groups.has(key)) groups.set(key, [])
        groups.get(key)!.push({ edgeIdx: i, end: 'source' })
      }
    }
  }

  // For each group with multiple endpoints, spread them evenly
  for (const [key, refs] of groups) {
    ...
    // Available spread range along the node edge
    const edgeLength = isHorizontalSide ? node.width : node.height
    // Use 80% of the node edge for spreading, centered
    const usable = edgeLength * 0.8
    const start = (edgeLength - usable) / 2
    const step = refs.length > 1 ? usable / (refs.length - 1) : 0

    // Sort refs by their current position along the spread axis for stability
    refs.sort((a, b) => {
      const pa = getEndpoint(edges[a.edgeIdx]!, a.end)
      const pb = getEndpoint(edges[b.edgeIdx]!, b.end)
      return isHorizontalSide ? pa.x - pb.x : pa.y - pb.y
    })

    for (let j = 0; j < refs.length; j++) {
      const ref = refs[j]!
      const edge = edges[ref.edgeIdx]!
      const offset = start + step * j

      if (ref.end === 'target') {
        const ptIdx = edge.points.length - 1
        const prevIdx = edge.points.length - 2
        if (isHorizontalSide) {
          const newX = node.x + offset
          edge.points[ptIdx]!.x = newX
          // Also adjust the preceding bend point to maintain orthogonal path
          if (prevIdx >= 0) edge.points[prevIdx]!.x = newX
        } else {
          const newY = node.y + offset
          edge.points[ptIdx]!.y = newY
          if (prevIdx >= 0) edge.points[prevIdx]!.y = newY
        }
      } else {
        const prevIdx = 1
        if (isHorizontalSide) {
          const newX = node.x + offset
          edge.points[0]!.x = newX
          if (prevIdx < edge.points.length) edge.points[prevIdx]!.x = newX
        } else {
          const newY = node.y + offset
          edge.points[0]!.y = newY
          if (prevIdx < edge.points.length) edge.points[prevIdx]!.y = newY
        }
      }
    }
  }
...
function getEndpoint(edge: PositionedEdge, end: 'source' | 'target'): Point {
  return end === 'target'
    ? edge.points[edge.points.length - 1]!
    : edge.points[0]!
}

/** Detect which side of a node a point is on (within 1px tolerance) */
function detectSide(point: Point, node: PositionedNode): 'top' | 'bottom' | 'left' | 'right' | null {
  const tol = 2
  if (Math.abs(point.y - node.y) < tol) return 'top'
  if (Math.abs(point.y - (node.y + node.height)) < tol) return 'bottom'
  if (Math.abs(point.x - node.x) < tol) return 'left'
  if (Math.abs(point.x - (node.x + node.width)) < tol) return 'right'
  return null
}
