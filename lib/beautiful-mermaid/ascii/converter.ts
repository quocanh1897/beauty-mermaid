...
export function convertToAsciiGraph(parsed: MermaidGraph, config: AsciiConfig): AsciiGraph {
  ...
  for (const [id, mNode] of parsed.nodes) {
    const asciiNode: AsciiNode = {
      ...
      // Strip HTML tags (<br/>, <i>, <b>, etc.) — ASCII can't render them.
      displayLabel: stripHtmlTags(mNode.label),
      index,
      gridCoord: null,
      drawingCoord: null,
      drawing: null,
  ...
  for (const mEdge of parsed.edges) {
    ...
    edges.push({
      from,
      to,
      text: stripHtmlTags(mEdge.label ?? ''),
      path: [],
      labelLine: [],
      startDir: { x: 0, y: 0 },
      endDir: { x: 0, y: 0 },
...
/** Strip HTML tags and convert <br> to spaces for ASCII display */
